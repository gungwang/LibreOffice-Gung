param(
	...
	[string]$InitialValue = "12345",
	[string]$ExpectedToolId = "Calc.ToggleBold",
	[string]$Provider,
	[string]$Model,
...
$resolvedUserProfileDir = if ($UserProfileDir) {
	...
	[System.IO.Path]::GetFullPath((Join-Path $projectRootPath "build\lo-vfy-calc-safefmt"))
}

$probeScriptPath = Join-Path $PSScriptRoot "verify_calc_safe_formatting.py"
