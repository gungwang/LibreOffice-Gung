param(
	...
	[string]$Prompt = "Rewrite this selection in a more formal business tone.",
	[string]$InitialSelection = "we gotta ship this fast.",
...
$resolvedUserProfileDir = if ($UserProfileDir) {
	...
	[System.IO.Path]::GetFullPath((Join-Path $projectRootPath "build\lo-profile-verify-openrouter-writer-proposal"))
}

$probeScriptPath = Join-Path $PSScriptRoot "verify_sidebar_openrouter_writer_proposal.py"
