param(
	...
	[string]$ExpectedToolId = "Writer.ToggleBold",
	[string]$Provider,
	[string]$Model,
...
$resolvedUserProfileDir = if ($UserProfileDir) {
	...
	[System.IO.Path]::GetFullPath((Join-Path $projectRootPath "build\lo-vfy-safefmt"))
}

$probeScriptPath = Join-Path $PSScriptRoot "verify_safe_formatting.py"
