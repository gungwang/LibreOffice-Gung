param(
	...
	[string]$Prompt = "Make this bold.",
	[string]$InitialText = "Hello Draw",
	[string]$ExpectedToolId = "Draw.ToggleBold",
	[string]$Provider,
	[string]$Model,
...
$resolvedUserProfileDir = if ($UserProfileDir) {
	...
	[System.IO.Path]::GetFullPath((Join-Path $projectRootPath "build\lo-vfy-draw-safefmt"))
}

$probeScriptPath = Join-Path $PSScriptRoot "verify_draw_safe_formatting.py"
